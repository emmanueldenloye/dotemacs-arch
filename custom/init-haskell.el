...
(use-package shm
  ...
  :defer t
  :init
  (add-to-list 'load-path "/home/eod/structured-haskell-mode/elisp/"))
...
(use-package ghc
  ...
  ;; (remove-hook 'haskell-mode-hook (lambda () (haskell-indentation-mode -1)))
  )
...
(use-package haskell-mode
  ...
  (let
    ...
    (setenv "PATH"
            (concat my-cabal-path path-separator
                    (getenv "PATH")))
    (add-to-list 'exec-path my-cabal-path))
  (setq exec-path
        (cons "/usr/bin"
              (remove "/usr/bin" exec-path)))
  ;; To get eta working.
  (let
      ((my-eta-path
        (expand-file-name "~/.local/bin")))
    (setenv "PATH"
            (concat my-eta-path path-separator
                    (getenv "PATH")))
    (add-to-list 'exec-path my-eta-path))
  (setq ghc-ghc-options '("-idir1" "-idir2"))
  (setq haskell-process-args-ghci
        (split-string "-ferror-spans -freverse-errors"))
  (add-hook 'ghc-core-mode-hook
            (lambda
              ()
  ...
  (defun haskell-company-delay-set ()
    (setq-local company-idle-delay 1))
  (defun haskell-no-indentation-mode ()
    (haskell-indentation-mode -1))
  (dolist
      (hook
       '(haskell-company-delay-set
         haskell-no-indentation-mode
         subword-mode
         auto-fill-mode
  ...
  (bind-keys :map haskell-mode-map
	     ...
	     ("C-M-d" . sp-down-sexp)
	     ("<f12>" . haskell-simple-compile-or-run))
...
(defun haskell-mode-turn-on-shm-case-split
    ()
  (when
      (fboundp 'structured-haskell-mode)
    (require 'shm-case-split)
    (define-key haskell-mode-map
      (kbd "C-c s")
      'shm/case-split-completing-read)))

(defun shm-reformat-current-node
    (&optional node start)
...
(defun eod-ghc-insert-template-or-signature
    (&optional arg)
  (interactive)
  (setq arg
        (or arg 1))
  ...
  (let
    ...
    (save-excursion
      (goto-char
       (point-min))
      (while
          (search-forward-regexp "error:$" nil t)
        (setq count
              (1+ count))))
    (message "The number of errors encountered: %s" count)))

(defun haskell-simple-compile-or-run
    (arg)
      ...
      (cond
       ...
       ((eq 16 arg)
        (let
            ((options
              (format "%s"
                      (read-from-minibuffer "Enter compilation options (if any): "))))
          (async-shell-command
           (concat "ghc " options " " file))))
       (t
        (async-shell-command
         (concat "ghc " file)))))))
...
;; (defadvice sp-split-sexp
;;     (around haskell-insert-plus-on-split activate)
;;   (-if-let
;;       (bounds
;;        (eod-haskell-split-bracket))
;;       (let
;;           ((nsexps
;;             (-
;;              (length
...
(defadvice sp-join-sexp
    (before haskell-insert-fix activate)
  (let
      ((searchbound
        (save-excursion
...
(defun  haskell-auto-insert-module-template
  ...
  (interactive)
  (when (equal major-mode 'haskell-mode)
    (when-let
       ((name
     ...
     (save-excursion
       (save-restriction
         ...
         (goto-char
          (point-min))
         (while
             (search-forward-regexp "\\bundefined\\b" nil t)
           (progn
             (delete-region
              (save-excursion
                (backward-word 1)
...
(defadvice shm-reformat-decl
    (after reinsert-undefined activate)
  (shm-reinsert-undefined-slots-after-fill))

(defadvice shm/backward-node
    (around in-comment-or-string activate)
  (if
      (or
       (shm-in-comment)
       (shm-in-string))
...
(defadvice hindent-reformat-decl-or-fill
    (after reinsert-undefined activate)
  (shm-reinsert-undefined-slots-after-fill))
...
(defun shm-push-current-data-constructor-down
    (&optional arg)
...
data Test = A | C | (point)B"
  ...
  (let*
      ((regions
        (cond
         ((shm-find-current-field-member-index-at-point)
         ...
         (t
          (error "Not a valid point for transposition."))))
       (end
        (length regions))
       (ind
        (or
...
(defun shm-push-current-data-constructor-up
    (&optional arg)
  "Transpose the current data constructor with the data
          ...
          (delq nil
                (mapcar
                 (lambda
                   (b)
                   (if
                       (within-interval bounds-data-index b)
                       b nil))
                 field-list)))
      field-list)))

(defun within-interval
    (data field)
      ...
      ((data-start1
       ...
       (field-end1
        (marker-position
         (cdr field))))
    (and
     (< data-start1 field-start1)
     (> data-end1 field-start1)
     (< data-start1 field-end1)
     (> data-end1 field-end1))))
...
(defun shm-transpose-data-constructors
    (m n)
  "Transpose the mth and nth data constructors. This function
will silently fail if M or N are not valid indices (the first
...
(defun shm-transpose-data-constructors-helper
    (bounds m1 m2 m3 m4)
  (let
      ((commentEndA
        (let
                  ...
                  (if
                    ...
                    (progn
                      ...
                      (let*
                        ...
                        (insert text)
                        (point))))
                                        ;The text will be unnormalized for now. May fix this later.
                (let
                    ((sub
                      (buffer-substring-no-properties orig
                                                      (point))))
                  (if
                      (-all-p
                       (lambda
      ...
      (let
        ...
        (if parsed-ast
            (progn
              (when
                  (bound-and-true-p structured-haskell-repl-mode)
                (shm-font-lock-region
                 (car bounds)
                 (cdr bounds)))
              (let
                  ((ast
                    (shm-get-nodes parsed-ast
                                   (car bounds)
                                   (cdr bounds))))
                (if ast
                    (progn
...
the point should be at the beginning of an evaporating undefined."
  ...
  (unless
    ...
    (shm/goto-topmost-parent))
  ;; this is for newtype declarations.
  (let
      ((current
        (shm-current-node))
       (line
     ...
     ((eq
      ...
      (if-let
        ...
        (progn
          (shm/forward-node)
          (comment-forward 1)
          ;; comments don't get skipped over by shm/forward-node
          (backward-char)
          ...
          (save-excursion
            ...
            (insert ")")))))
     (t
      (message "The point is not contained within a data type declaration.")))))
...
(defadvice hindent-reformat-decl-or-fill
    (after fill-string activate)
  (shm-fill-string))

(defadvice hindent-reformat-decl-or-fill
    (around no-shm-scan activate)
...
(defadvice shm/double-quote
    (around wrap-with-quotes activate)
  (if
      (use-region-p)
      (progn
        (save-excursion
...
(defadvice shm/open-paren
    (around check-for-at-symbol activate)
  (if
      (looking-back "@"
                    (save-excursion
...
(defadvice undo-tree-undo
    (after check-if-word-after-is-undefined activate)
  (when
      (and
       (eq major-mode 'haskell-mode)
       structured-haskell-mode
       ...
       (text
        (delete-and-extract-region
         (shm-node-start current-node)
                                        ;I might want to use shm opinionated killing and yanking functions
...
(defadvice shm/yank
    (after shm-exchange-point-and-mark activate)
  (exchange-point-and-mark)
  (deactivate-mark))

(defadvice shm/insert-undefined
    (before shm-clear-active-region activate)
  (if
      (use-region-p)
      (delete-region
       (region-beginning)
  ...
  (let*
    ...
    (unwind-protect
        (with-helm-show-completion beg end
          (helm :sources
                (helm-make-source "Haskell Interactive Mode History"
...
(with-eval-after-load 'helm-mode
  (with-eval-after-load 'haskell-mode
    (defclass helm-haskell-interactive-mode-history-source
            ...
            (lambda
              ...
              (with-current-buffer
                ...
                (mapc
                 ...
                 (with-current-buffer
                     helm-current-buffer
                   (nreverse haskell-interactive-mode-history))))))
      (nomark :initform t)
...
(defun helm-shm-faces
    ()
  "Choose a current/quarantine face pair."
  (interactive)
  (helm
   :sources
   (helm-build-sync-source "SHM current/quarantine face alist"
     :candidates
     '(("Light Theme #eee8d5 #fffacd" .
        ("#eee8d5" "#fffacd"))
       ("Dark Theme #373737 #443333" .
        ("#373737" "#443333")))
     :fuzzy-match t
     :action
     'shm-set-face
  ...
  (if
      (not
       (and
        (eq major-mode 'haskell-mode)
        structured-haskell-mode))
      (error "The current buffer's major mode is not in
      `haskell-mode' or `haskell-mode' is the current buffer's
      major mode but `structured-haskell-mode' is not on.")
    (let
        ((current-faces
          (split-string "#eee8d5 #373737"))
         (quarantine-faces
          (split-string "#fffacd #443333")))
      (if
          (not
           (and
            (member
             (face-background 'shm-current-face)
             current-faces)
            (member
             (face-background 'shm-quarantine-face)
             quarantine-faces)))
          (call-interactively 'helm-shm-faces)
        (progn
          (set-face-background 'shm-current-face
                               (cdr
                                (find-if
                                 (lambda
                                   (face)
                                   (string=
                                    (car face)
                                    (face-background 'shm-current-face)))
                                 (cl-mapcar 'cons current-faces
                                            (reverse current-faces)))))
          (set-face-background 'shm-quarantine-face
                               (cdr
                                (find-if
                                 (lambda
                                   (face)
                                   (string=
                                    (car face)
                                    (face-background 'shm-quarantine-face)))
                                 (cl-mapcar 'cons quarantine-faces
                                            (reverse quarantine-faces))))))))))
...
(defun shm/case-split-completing-read
...
(defun haskell-clear-to-one-line-between-functions
  ...
  (when
      (and
       (fboundp 'haskell-decl-scan-mode)
    ...
    (save-excursion
      ...
      (while
        ...
        (goto-char
         (get-desired-point 'min
                            '(end-of-defun forward-paragraph)))
        (unless
            (blank-line-p)
                                        ;This is honestly a bit of a hack!
          (when
              (save-excursion
                (forward-line)
                (blank-line-p))
...
(with-eval-after-load 'haskell-mode
 (define-key haskell-mode-map
  (kbd "M-p")
  'haskell-highlight-symbol-prev-or-prev-error))
...
(with-eval-after-load 'haskell-mode
  (define-key haskell-mode-map
    (kbd "M-n")
    'haskell-highlight-symbol-next-or-next-error))

(defun eod-haskell-process-load-file
    (arg)
...
(defun shm-collect-matches
    (regexp reverse-search bound col)
...
working with nested if statements."
  ...
  (save-excursion
    (let
        (collect)
      (while
          (funcall searchfunc regexp bound t)
        (if
            (= col
               (save-excursion
                 (goto-char
...
(defadvice sp-splice-sexp-killing-around
    (around fix-indentation activate)
  ad-do-it
  (if
...
(defun haskell-add-timing-code
    ()
  (interactive)
  (let
      ((imports
        '("import System.CPUTime" "import Text.Printf" "import Control.Exception"))
       (code "\ntime :: IO a -> IO a\ntime a = do\n  start <- getCPUTime\n  v <- a\n  end <- getCPUTime\n  let diff = (fromIntegral (end - start)) / (10 ^ 12)\n  printf \"Computation time: %0.3f sec\\n\" (diff :: Double)\n  return v" ))
    (save-excursion
      (haskell-navigate-imports)
      (haskell-navigate-imports)
      (insert (mapconcat 'identity imports "\n"))
      (insert "\n")
      (insert code)
      (insert "\n"))))

(defadvice hindent-reformat-buffer (around no-shm-scan activate)
  (if structured-haskell-mode
      (unwind-protect
       (progn
         (structured-haskell-mode -1)
...
(defun temp-haskell-load-or-reload
    (command)
  (interactive)
  (save-excursion
    (message "here temp-haskell-load-or-reload")
    (when
      (member command
              '(load reload))
    (let
...
;; (ad-deactivate 'haskell-process-load-file)
;; (ad-deactivate 'haskell-process-reload)
;; (ad-deactivate 'haskell-process-file-loadish)

;; (defadvice haskell-process-load-file
;;     (around workaround-load activate)
;;   (unless
;;       (ignore-errors ad-do-it)
;;     (temp-haskell-load-or-reload 'load)))

;; (defadvice haskell-process-reload
;;     (around workaround-reload activate)
;;   (unless
;;       (ignore-errors ad-do-it)
;;     (temp-haskell-load-or-reload 'reload)))

;; (defadvice haskell-process-file-loadish
;;     (around save-point activate)
;;   (save-excursion
;;     ad-do-it))

(defun eod-haskell-goto-where
    ()
  (interactive)
  "We are going to be really lazy about this."
  (if structured-haskell-mode
    (shm/goto-where)
    (unwind-protect
       (progn
...
(with-eval-after-load 'haskell-mode
  (define-key haskell-mode-map
    (kbd "C-c C-w")
    'eod-haskell-goto-where))

(defun haskell-goto-start-of-imports ()
  (progn
    (goto-char (point-min))
    (re-search-forward "^import" nil t nil)
    (backward-word)
    (point)))

(defun collect-number-of-import-groups
    ()
  (save-excursion
    (haskell-goto-start-of-imports)
    (progn
      (while
          (re-search-forward "^import" nil t nil))
      (forward-line 1)
...
(defun eod-haskell-insert-import (modules &optional add-qualified)
  (interactive "sInsert module: \n")
  (when (equal major-mode 'haskell-mode)
    (setq add-qualified (or add-qualified))
    (save-excursion
      (haskell-goto-start-of-imports)
      (open-line 1)
      (insert
       ...
       (lambda (module)
         (concat
          ...
          (if add-qualified
              "qualified "
            "")
          (mapconcat 'capitalize (split-string (string-trim module) "\\.") ".")))
       (split-string modules ";")
       "\n")))))


(defun probe-shm-constructor ()
  (let* ((current-pair (shm-current-node-pair))
         (current (cdr current-pair)))
    (shm-node-cons current)))

(defadvice hippie-expand
    (around disable-subword-mode activate)
  (not-if (eq major-mode 'haskell-mode)
      ad-do-it
    (subword-mode -1)
    (unwind-protect
        ad-do-it
