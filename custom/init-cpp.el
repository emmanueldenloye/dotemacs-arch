...
(defun opencv-compile (run)
  ...
  (-if-let*
      ...
      (progn
        (save-buffer)                    ;sanity check
        (let ((runstr (concat "&& " "./" (file-name-base (buffer-file-name)))))
          (async-shell-command
           (string-join
...
(with-eval-after-load 'c++-mode
 (define-key c++-mode-map (kbd "C-c C-r") 'opencv-compile))

(defun c++-clear-to-one-line-between-functions ()
  (interactive)
  (save-excursion
    (goto-char (point-min))
    (while (search-forward-regexp "\\(^#include\\|^using\\)" nil t)
      (forward-line))
