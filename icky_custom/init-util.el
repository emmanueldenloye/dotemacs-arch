(with-current-buffer (get-buffer-create "*Require Times*")
  (insert "| feature | timestamp | elasped |\n")
  (insert "|---------+-----------+---------|\n"))
  ...
  (let ((elasped)
    ...
    (prog1
      ...
      (unless-loaded
       (with-current-buffer (get-buffer-create "*Require Times*")
         (goto-char (point-max))
         (setq elasped (float-time (time-subtract (current-time) start)))
         (insert  (format "| %s | %s | %f |\n"
                          feature
...
(let ((base  (concat user-emacs-directory "/elisp")))
  ...
  (dolist (dir (directory-files abse t "^[^.]"))
...
(defun my-recompile-init ()
  ...
  (interactive)
  (byte-recompile-directory (concat user-emacs-directory "custom") 0)
